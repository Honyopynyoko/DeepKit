...
exports.AppModule = exports.AppModuleConfig = exports.ConfigurationInvalidError = void 0;
...
let moduleId = 0;
class AppModuleConfig extends injector_1.ConfigDefinition {
    constructor(config) {
        super(type_1.t.schema(config));
        this.config = config;
    }
}
exports.AppModuleConfig = AppModuleConfig;
class AppModule extends injector_1.InjectorModule {
    ...
    name = '', configValues = {}, setups = [], id = moduleId++) {
        ...
        if (options.config instanceof injector_1.ConfigDefinition) {
    ...
    getConfig() {
        ...
        try {
            Object.assign(this.config, type_1.jsonSerializer.for(this.options.config.schema).validatedDeserialize(config));
            return this.config;
        }
        catch (e) {
            if (e instanceof type_1.ValidationFailed) {
    ...
    setConfig(config) {
        ...
        if (this.options.config) {
            const configNormalized = type_1.jsonSerializer.for(this.options.config.schema).partialDeserialize(config);
            for (const option of this.options.config.schema.getProperties()) {
                if (!(option.name in config))
                    continue;
