...
exports.getBSONDecoder = exports.getRawBSONDecoder = void 0;
const core_1 = require("@deepkit/core");
const type_1 = require("@deepkit/type");
const bson_parser_1 = require("./bson-parser");
const bson_typeguards_1 = require("./bson-typeguards");
const continuation_1 = require("./continuation");
const utils_1 = require("./utils");
function createPropertyConverter(setter, property, compiler, parentProperty) {
    ...
    const defaultValue = !property.hasDefaultValue && property.defaultValue !== undefined ?
        ...
        : 'undefined';
    const nullCheck = `
    if (elementType === ${6 /* UNDEFINED */}) {
        if (${property.isOptional}) ${setter} = ${defaultValue};
    } else if (elementType === ${10 /* NULL */}) {
        if (${property.isOptional}) ${setter} = ${defaultValue};
    ...
    if (property.type === 'string') {
        ...
        if (property.isOptional || property.isNullable) {
            return `
                if (${property.isOptional} && (elementType === ${6 /* UNDEFINED */} || elementType === ${10 /* NULL */})) {
                    ${setter} = ${defaultValue};
                } else if (${property.isNullable} && elementType === ${10 /* NULL */}) {
                    ${setter} = null;
                } else {
                    ${setter} = ${literalValue};
                    seekElementSize(elementType, parser);
        ...
        return `
        if (elementType === ${2 /* STRING */} || elementType === ${1 /* NUMBER */} || elementType === ${16 /* INT */} || elementType === ${18 /* LONG */}) {
        ...
        return `
            if (elementType === ${5 /* BINARY */}) {
                parser.eatUInt32(); //size
                const subType = parser.eatByte();
                if (subType !== ${utils_1.BSON_BINARY_SUBTYPE_UUID}) throw new Error('${property.name} BSON binary type invalid. Expected UUID(4), but got ' + subType); 
    ...
    `;
        ...
        if (schema.decorator) {
            ...
            return `
            //decorated
            if (elementType !== ${6 /* UNDEFINED */} && elementType !== ${10 /* NULL */}) {
                ${decoratedVar} = {};
        ...
        const discriminants = [];
        for (const unionType of type_1.getSortedUnionTypes(property, bson_typeguards_1.bsonTypeGuards)) {
...
function createSchemaDecoder(schema) {
...
function getRawBSONDecoder(schema) {
    schema = schema instanceof type_1.ClassSchema ? schema : type_1.getClassSchema(schema);
    if (schema.jit.rawBson)
        return schema.jit.rawBson;
    schema.jit.rawBson = createSchemaDecoder(schema);
    return schema.jit.rawBson;
}
exports.getRawBSONDecoder = getRawBSONDecoder;
function getBSONDecoder(schema) {
    const fn = getRawBSONDecoder(schema);
    schema = schema instanceof type_1.ClassSchema ? schema : type_1.getClassSchema(schema);
    if (schema.jit.bsonEncoder)
        return schema.jit.bsonEncoder;
    schema.jit.bsonEncoder = (bson, offset = 0) => {
        return fn(new bson_parser_1.ParserV2(bson, offset));
    };
    return schema.jit.bsonEncoder;
}
exports.getBSONDecoder = getBSONDecoder;
//# sourceMappingURL=bson-jit-parser.js.map