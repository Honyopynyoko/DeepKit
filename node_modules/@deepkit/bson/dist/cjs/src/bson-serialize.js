...
exports.getBSONSizer = exports.getBSONSerializer = exports.serialize = exports.Writer = exports.createBSONSizer = exports.getValueSize = exports.stringByteLength = exports.uuidStringToByte = exports.hexToByte = exports.JS_INT_MIN = exports.JS_INT_MAX = exports.createBuffer = void 0;
...
function getValueSize(value) {
    ...
    else if (core_1.isArray(value)) {
        let size = 4; //object size
        for (let i = 0; i < value.length; i++) {
            size += 1; //element type
            size += utils_1.digitByteSize(i); //element name
            size += getValueSize(value[i]);
        }
        size += 1; //null
        return size;
    }
    else if (model_1.isUUID(value)) {
        return 4 + 1 + 16;
    }
    else if (model_1.isObjectId(value)) {
        return 12;
    }
...
function getPropertySizer(schema, compiler, property, accessor, jitStack) {
    ...
    if (property.type === 'array') {
        compiler.context.set('digitByteSize', utils_1.digitByteSize);
        const isArrayVar = compiler.reserveVariable('isArray', core_1.isArray);
        const unpopulatedSymbolVar = compiler.reserveVariable('unpopulatedSymbol', type_1.unpopulatedSymbol);
        ...
        if (typeof ${accessor} === 'number') {
            if (Math.floor(${accessor}) === ${accessor}) {
                ...
                if (${accessor} >= ${BSON_INT32_MIN} && ${accessor} <= ${BSON_INT32_MAX}) {
                    //32bit
                    size += 4;
                } else if (${accessor} >= ${exports.JS_INT_MIN} && ${accessor} <= ${exports.JS_INT_MAX}) {
                    //double, 64bit
                    size += 8;
                } else {
                    //long
        ...
        const unpopulatedSymbolVar = compiler.reserveVariable('unpopulatedSymbol', type_1.unpopulatedSymbol);
        compiler.context.set('isObject', core_1.isObject);
        compiler.context.set(sizer, sizerFn);
        compiler.context.set('UUIDSymbol', model_1.UUIDSymbol);
        compiler.context.set('ObjectIdSymbol', model_1.ObjectIdSymbol);
        ...
        compiler.context.set('isObjectId', model_1.isObjectId);
        code = `if ('string' === typeof ${accessor}|| isObjectId(${accessor})) size += 12;`;
    }
    else if (property.type === 'uuid') {
        compiler.context.set('isUUID', model_1.isUUID);
        ...
        const discriminants = [];
        for (const unionType of type_1.getSortedUnionTypes(property, type_1.jsonTypeGuards)) {
 ...
 */
function createBSONSizer(schema, jitStack = new type_1.JitStack()) {
    ...
    compiler.context.set('_global', type_1.getGlobalStore());
    compiler.context.set('UnpopulatedCheck', type_1.UnpopulatedCheck);
...
class Writer {
    ...
    writeBigInt(value) {
        ...
        this.writeByte(utils_1.BSON_BINARY_SUBTYPE_BIGINT);
    ...
    writeUUID(value) {
        value = value instanceof model_1.UUID ? value.id : value;
        this.writeUint32(16);
        this.writeByte(utils_1.BSON_BINARY_SUBTYPE_UUID);
    ...
    write(value, nameWriter) {
        if ('boolean' === typeof value) {
            if (nameWriter) {
                this.writeByte(8 /* BOOLEAN */);
                nameWriter();
            }
            ...
            if (nameWriter) {
                this.writeByte(9 /* DATE */);
                nameWriter();
            }
            this.writeUint32((value.valueOf() % utils_1.TWO_PWR_32_DBL_N) | 0); //low
            this.writeUint32((value.valueOf() / utils_1.TWO_PWR_32_DBL_N) | 0); //high
        }
        else if (model_1.isUUID(value)) {
            if (nameWriter) {
                this.writeByte(5 /* BINARY */);
                nameWriter();
            }
            ...
            this.writeByte(utils_1.BSON_BINARY_SUBTYPE_DEFAULT);
            ...
            if (nameWriter) {
                this.writeByte(6 /* UNDEFINED */);
                nameWriter();
            }
        }
        else if (value === null) {
            if (nameWriter) {
                this.writeByte(10 /* NULL */);
                nameWriter();
            }
        }
        else if (core_1.isObject(value)) {
            if (nameWriter) {
                this.writeByte(3 /* OBJECT */);
                nameWriter();
            }
            const start = this.offset;
            this.offset += 4; //size
...
function getNameWriterCode(property) {
    const nameSetter = [];
...
function getPropertySerializerCode(schema, compiler, property, accessor, jitStack, nameAccessor) {
    ...
    if (property.type === 'class') {
        ...
        compiler.context.set('TWO_PWR_32_DBL_N', utils_1.TWO_PWR_32_DBL_N);
        code = `
        if (${accessor} instanceof Date) {
            writer.writeByte(${9 /* DATE */});
            ${nameWriter}
            if (!(${accessor} instanceof Date)) {
        ...
        compiler.context.set('isUUID', model_1.isUUID);
        compiler.context.set('UUID', model_1.UUID);
        code = `
        if ('string' === typeof ${accessor} || isUUID(${accessor})) {
            writer.writeByte(${5 /* BINARY */});
            ${nameWriter}
        ...
        code = `
            if ('bigint' === typeof ${accessor}) {
                ...
                if (Math.floor(${accessor}) === ${accessor}) {
                    ...
                    if (${accessor} >= ${BSON_INT32_MIN} && ${accessor} <= ${BSON_INT32_MAX}) {
                        //32bit
                        writer.writeByte(${16 /* INT */});
                        ${nameWriter}
                        writer.writeInt32(${accessor});
                    } else if (${accessor} >= ${exports.JS_INT_MIN} && ${accessor} <= ${exports.JS_INT_MAX}) {
                        //double, 64bit
                        writer.writeByte(${1 /* NUMBER */});
                        ${nameWriter}
...
function createBSONSerialize(schema, jitStack = new type_1.JitStack()) {
    const compiler = new core_1.CompilerContext();
    const prepared = jitStack.prepare(schema);
    compiler.context.set('_global', type_1.getGlobalStore());
    compiler.context.set('UnpopulatedCheck', type_1.UnpopulatedCheck);
...
//# sourceMappingURL=bson-serialize.js.map