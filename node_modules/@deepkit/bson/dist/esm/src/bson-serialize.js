 ...
 */
import { CompilerContext, isArray, isObject, toFastProperties } from '@deepkit/core';
import { getClassSchema, getGlobalStore, getSortedUnionTypes, JitStack, jsonTypeGuards, UnpopulatedCheck, unpopulatedSymbol } from '@deepkit/type';
import { seekElementSize } from './continuation';
import { isObjectId, isUUID, ObjectIdSymbol, UUID, UUIDSymbol } from './model';
import { BSON_BINARY_SUBTYPE_BIGINT, BSON_BINARY_SUBTYPE_DEFAULT, BSON_BINARY_SUBTYPE_UUID, digitByteSize, TWO_PWR_32_DBL_N } from './utils';
...
export function hexToByte(hex, index = 0, offset = 0) {
...
export function uuidStringToByte(hex, index = 0) {
    let offset = 0;
...
export function getValueSize(value) {
 ...
 */
export function createBSONSizer(schema, jitStack = new JitStack()) {
...
function createBSONSerialize(schema, jitStack = new JitStack()) {
    const compiler = new CompilerContext();
    const prepared = jitStack.prepare(schema);
    compiler.context.set('_global', getGlobalStore());
    compiler.context.set('UnpopulatedCheck', UnpopulatedCheck);
 ...
 */
export function getBSONSerializer(schema) {
...
export function getBSONSizer(schema) {
    schema = getClassSchema(schema);
    const jit = schema.jit;
    if (jit.bsonSizer)
