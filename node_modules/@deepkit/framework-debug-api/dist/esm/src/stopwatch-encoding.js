...
export function encodeFrameData(dataItems) {
    //<id uint32><worker uint8><bson document>
    let size = 0;
    for (const data of dataItems) {
        const schema = categorySchemas[data.category];
        if (!schema)
            throw new Error(`Frame category ${FrameCategory[data.category]} has no schema declared.`);
...
export function decodeFrameData(buffer) {
...
//# sourceMappingURL=stopwatch-encoding.js.map