...
exports.LiveDatabase = exports.LiveQuery = void 0;
...
class SubscriptionHandler {
    ...
    subscribeEntity() {
        ...
        this.entitySubscription = this.broker.entityChannel(this.classSchema).subscribe((message) => {
            if (message.type === broker_1.EntityChannelMessageType.remove) {
                for (const id of message.ids) {
                    this.rmSentState(id);
                }
                this.connection.createMessageBuilder()
                    .composite(rpc_1.RpcTypes.Entity)
                    .add(rpc_1.RpcTypes.EntityRemove, rpc_1.rpcEntityRemove, {
                    entityName: entityName,
                    ids: message.ids,
                }).send();
                return;
            }
            if (message.type === broker_1.EntityChannelMessageType.patch && this.needsToBeSend(message.id, message.version)) {
                this.setSent(message.id, message.version);
                this.connection.createMessageBuilder()
                    .composite(rpc_1.RpcTypes.Entity)
                    .add(rpc_1.RpcTypes.EntityPatch, rpc_1.rpcEntityPatch, {
                    entityName: entityName,
                    id: message.id,
...
class LiveDatabaseQueryModel extends orm_1.DatabaseQueryModel {
    constructor() {
        super(...arguments);
...
function extractModelData(classSchema, model, result) {
    result.push({
        classSchema,
        filter: model.filter || {},
        fields: model.filter ? orm_1.exportQueryFilterFieldNames(classSchema, model.filter) : [],
    });
...
function extractJoinedClassSchemaInfos(model, result) {
    for (const join of model.joins) {
        if (join.propertySchema.backReference && join.propertySchema.backReference.via) {
            const schema = type_1.getClassSchema(type_1.resolveClassTypeOrForward(join.propertySchema.backReference.via));
            result.push({
                classSchema: schema,
                filter: {},
                fields: [],
...
class LiveCollection {
    constructor(collection, model, broker, database, subscriptionHandler, connection) {
        ...
        this.classSchema = type_1.getClassSchema(this.collection.classType);
        this.rootFields = orm_1.exportQueryFilterFieldNames(this.classSchema, model.filter || {});
    ...
    async updateCollection(databaseChanged = false) {
        ...
        this.lastUpdatePromise = core_1.asyncOperation((resolve) => {
    ...
    async startSync() {
        ...
        let currentQuery = orm_1.replaceQueryFilterParameter(this.classSchema, this.model.filter || {}, this.model.parameters);
        ...
        this.entitySubscription = await this.broker.entityChannel(this.classSchema).subscribe(async (message) => {
            ...
            if (message.type === broker_1.EntityChannelMessageType.add) {
                if (this.knownIDs.has(message.id) || !utils_1.findQuerySatisfied(message.item, currentQuery)) {
                    return;
                }
                if (requiresFullUpdate(this.collection)) {
                    await this.updateCollection(true);
            ...
            if (message.type === broker_1.EntityChannelMessageType.patch) {
                const querySatisfied = utils_1.findQuerySatisfied(message.item, currentQuery);
...
class LiveQuery extends orm_1.BaseQuery {
    ...
    async findOneOrUndefined() {
        ...
        return new rpc_1.EntitySubject(item, () => {
    ...
    count() {
        const rootFields = orm_1.exportQueryFilterFieldNames(this.classSchema, this.model.filter || {});
        let currentQuery = orm_1.replaceQueryFilterParameter(this.classSchema, this.model.filter || {}, this.model.parameters);
        ...
        return new rxjs_1.Observable((observer) => {
            ...
            (async () => {
                ...
                rootEntitySub = await this.broker.entityChannel(this.classSchema).subscribe((message) => {
                    if (message.type === broker_1.EntityChannelMessageType.add) {
                        if (!knownIDs.has(message.id) && utils_1.findQuerySatisfied(message.item, currentQuery)) {
                            counter++;
                    ...
                    if (message.type === broker_1.EntityChannelMessageType.patch) {
                        ...
                        else if (!knownIDs.has(message.id) && utils_1.findQuerySatisfied(message.item, currentQuery)) {
                            counter++;
                            knownIDs.add(message.id);
                            observer.next(counter);
                        }
    ...
    async find() {
        const collection = new rpc_1.Collection(this.classSchema.classType);
        ...
        return new rxjs_1.Observable((observer) => {
            ...
            (async () => {
                sub = await this.broker.entityChannel(this.classSchema).subscribe((message) => {
                    if (message.type === broker_1.EntityChannelMessageType.add && utils_1.findQuerySatisfied(message.item, filter)) {
...
let LiveDatabase = class LiveDatabase {
    ...
    getSubscriptionHandler(classType) {
        return this.subscriptionHandler.get(type_1.getClassSchema(classType));
    }
    ...
    query(classType) {
        return new LiveQuery(type_1.getClassSchema(classType), this.connection, this.databases.getDatabaseForEntity(classType), this.broker, this.subscriptionHandler.get(type_1.getClassSchema(classType)));
    }
};
...
exports.LiveDatabase = LiveDatabase;
