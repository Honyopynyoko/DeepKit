...
import { asyncOperation, CompilerContext, getClassName, urlJoin } from '@deepkit/core';
import { getClassSchema, getPropertyXtoClassFunction, jitValidateProperty, jsonSerializer, t, ValidationFailed } from '@deepkit/type';
// @ts-ignore
import formidable from 'formidable';
import querystring from 'querystring';
import { httpClass } from './decorator';
import { injectable, Tag, TagProvider, TagRegistry } from '@deepkit/injector';
...
export function parseRouteControllerAction(routeConfig) {
...
export function dotToUrlPath(dotPath) {
    if (-1 === dotPath.indexOf('.'))
        return dotPath;
    return dotPath.replace(/\./g, '][').replace('][', '[') + ']';
}
export class ParameterResolverTagProvider extends TagProvider {
    constructor() {
        super(...arguments);
        this.classTypes = [];
    }
...
export class RouteParameterResolverTag extends Tag {
...
let Router = class Router {
    ...
    static forControllers(controllers, tagRegistry = new TagRegistry()) {
    ...
    getRouteCode(compiler, routeConfig) {
        ...
        for (const parameter of parsedRoute.getParameters()) {
            if (parsedRoute.customValidationErrorHandling === parameter) {
                ...
                const converted = parameter.property.type === 'any' ? (v) => v : getPropertyXtoClassFunction(parameter.property, jsonSerializer);
                const validator = parameter.property.type === 'any' ? (v) => undefined : jitValidateProperty(parameter.property);
...
Router = __decorate([
    injectable(),
    __metadata("design:paramtypes", [HttpControllers,
        Logger,
        TagRegistry])
], Router);
export { Router };
