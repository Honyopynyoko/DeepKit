...
async function createDatabase(dbName = 'testing') {
    ...
    return new orm_1.Database(new adapter_1.MongoDatabaseAdapter('mongodb://localhost/' + dbName));
}
const items = 10000;
(async () => {
    ...
    for (let j = 1; j <= 15; j++) {
        ...
        await utils_1.bench(1, 'deepkit/orm insert', async () => {
        ...
        await utils_1.bench(10, 'deepkit/orm find', async () => {
            await query.find();
        });
        session.identityMap.clear();
        ...
        await utils_1.bench(1, 'deepkit/orm update', async () => {
            await session.commit();
        });
    }
    database.disconnect();
