...
import { BaseResponse } from '../command';
...
export class ScramAuth {
    ...
    parseStartPayload(payload) {
        ...
        if (result.i < 4096)
            throw new MongoError(`Server returned an invalid iteration count ${result.i}`);
        if (result.r.startsWith('nonce'))
            throw new MongoError(`Server returned an invalid nonce: ${result.r}`);
        return result;
    }
