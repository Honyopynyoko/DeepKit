...
exports.fakerFunctions = exports.findFaker = exports.findFakerForName = exports.getType = void 0;
function getType(v) {
    if ('string' === typeof v)
        return 'string';
    if ('number' === typeof v)
...
function findFakerForName(types, name, type) {
    for (const [fakeName, info] of Object.entries(types)) {
        if (info.type !== type)
            continue;
        const [p1, p2] = fakeName.toLowerCase().split('.');
        if (p2 && p2.includes(name))
            return fakeName;
        if (p2 && name.includes(p2))
            return fakeName;
    }
    return undefined;
}
exports.findFakerForName = findFakerForName;
function findFaker(types, property) {
