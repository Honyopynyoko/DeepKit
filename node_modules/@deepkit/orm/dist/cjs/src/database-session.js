...
exports.DatabaseSession = exports.DatabaseTransaction = exports.SessionClosedException = exports.DatabaseSessionRound = void 0;
const type_1 = require("./type");
const core_1 = require("@deepkit/core");
const type_2 = require("@deepkit/type");
const topsort_1 = require("@deepkit/topsort");
...
class DatabaseSessionRound {
    constructor(session, identityMap, emitter, logger) {
        ...
        this.global = type_2.getGlobalStore();
    }
    isInCommit() {
    ...
    getReferenceDependencies(item) {
        const result = [];
        const classSchema = type_2.getClassSchema(type_2.getClassTypeFromInstance(item));
        const old = this.global.unpopulatedCheck;
        this.global.unpopulatedCheck = type_2.UnpopulatedCheck.None;
        try {
            for (const reference of classSchema.references.values()) {
                ...
                if (reference.isArray) {
                    ...
                    if (v instanceof reference.getResolvedClassType() && !type_2.isReference(v))
                        result.push(v);
                }
            }
        }
    ...
    async doDelete(persistence) {
        for (const [classSchema, items] of utils_1.getClassSchemaInstancePairs(this.removeQueue.values())) {
            if (this.emitter.onDeletePre.hasSubscriptions()) {
                const event = new event_1.UnitOfWorkEvent(classSchema, this.session, items);
    ...
    async doPersist(persistence) {
        const sorter = new topsort_1.GroupArraySort();
        sorter.sameTypeExtraGrouping = true;
        sorter.throwOnNonExistingDependency = false;
        const unpopulatedCheck = type_2.getGlobalStore().unpopulatedCheck;
        type_2.getGlobalStore().unpopulatedCheck = type_2.UnpopulatedCheck.None;
        try {
            ...
            for (const group of groups) {
                const inserts = [];
                const changeSets = [];
                const classState = identity_map_1.getClassState(group.type);
                for (const item of group.items) {
                    const state = identity_map_1.getInstanceState(classState, item);
                    const errors = type_2.validate(classState.classSchema, item);
                    if (errors.length) {
                        throw new type_1.DatabaseValidationError(classState.classSchema, errors);
                    }
                ...
                if (inserts.length) {
                    ...
                    if (this.emitter.onInsertPre.hasSubscriptions()) {
                        const event = new event_1.UnitOfWorkEvent(group.type, this.session, inserts);
                    ...
                    if (doInsert) {
                        ...
                        if (this.emitter.onInsertPost.hasSubscriptions()) {
                            await this.emitter.onInsertPost.emit(new event_1.UnitOfWorkEvent(group.type, this.session, inserts));
                        }
                    }
                }
                if (changeSets.length) {
                    ...
                    if (this.emitter.onUpdatePre.hasSubscriptions()) {
                        const event = new event_1.UnitOfWorkUpdateEvent(group.type, this.session, changeSets);
                    ...
                    if (doUpdate) {
                        ...
                        if (this.emitter.onUpdatePost.hasSubscriptions()) {
                            await this.emitter.onUpdatePost.emit(new event_1.UnitOfWorkUpdateEvent(group.type, this.session, changeSets));
                        }
                    }
                }
            ...
            type_2.getGlobalStore().unpopulatedCheck = unpopulatedCheck;
        }
    }
}
exports.DatabaseSessionRound = DatabaseSessionRound;
class SessionClosedException extends core_1.CustomError {
}
exports.SessionClosedException = SessionClosedException;
...
class DatabaseSession {
    constructor(adapter, unitOfWorkEmitter = new event_1.UnitOfWorkDatabaseEmitter, queryEmitter = new event_1.QueryDatabaseEmitter(), logger = new logger_1.DatabaseLogger, stopwatch = new stopwatch_1.Stopwatch()) {
        ...
        this.identityMap = new identity_map_1.IdentityMap();
        this.rounds = [];
        this.commitDepth = 0;
        this.inCommit = false;
    ...
    getReference(classType, primaryKey) {
        const schema = type_2.getClassSchema(classType);
        const pk = identity_map_1.getNormalizedPrimaryKey(schema, primaryKey);
        return reference_1.getReference(schema, pk, this.identityMap);
    }
    getCurrentRound() {
    ...
    async hydrateEntity(item) {
        const classType = type_2.getClassTypeFromInstance(item);
        const classSchema = type_2.getClassSchema(classType);
        const pk = type_2.getPrimaryKeyExtractor(classSchema)(item);
    ...
    async commit() {
        ...
        if (this.unitOfWorkEmitter.onCommitPre.hasSubscriptions()) {
            const event = new event_1.UnitOfWorkCommitEvent(this);
