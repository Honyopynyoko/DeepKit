...
exports.Formatter = exports.setHydratedDatabaseSession = void 0;
const type_1 = require("@deepkit/type");
const core_1 = require("@deepkit/core");
const identity_map_1 = require("./identity-map");
const reference_1 = require("./reference");
function setHydratedDatabaseSession(item, hydrator) {
    const info = type_1.getReferenceInfo(item);
    if (info)
        info.hydrator = hydrator;
}
exports.setHydratedDatabaseSession = setHydratedDatabaseSession;
/**
...
class Formatter {
    constructor(rootClassSchema, serializer, hydrator, identityMap) {
        ...
        this.rootPkHash = type_1.getPrimaryKeyHashGenerator(this.rootClassSchema, this.serializer);
        this.rootClassState = identity_map_1.getClassState(this.rootClassSchema);
    }
    ...
    makeInvalidReference(item, classSchema, propertySchema) {
        Object.defineProperty(item, propertySchema.name, {
            ...
            get() {
                ...
                if (type_1.getGlobalStore().unpopulatedCheck === type_1.UnpopulatedCheck.Throw) {
                ...
                if (type_1.getGlobalStore().unpopulatedCheck === type_1.UnpopulatedCheck.ReturnSymbol) {
                    return type_1.unpopulatedSymbol;
                }
            },
            set(v) {
    ...
    getReferenceClass(classSchema) {
        ...
        if (Reference)
            return Reference;
        Reference = type_1.createReferenceClass(classSchema);
        if (this.hydrator) {
    ...
    getReference(classSchema, dbRecord, propertySchema, isPartial) {
        ...
        for (const property of foreignPrimaryFields) {
            const foreignKey = foreignPrimaryFields.length === 1 ? propertySchema.name : propertySchema.name + core_1.capitalize(property.name);
        ...
        if (allFilled === 0)
            return undefined;
        const ref = reference_1.getReference(foreignSchema, foreignPrimaryKey, isPartial ? undefined : this.identityMap, isPartial ? undefined : pool, this.getReferenceClass(foreignSchema));
        identity_map_1.getInstanceState(identity_map_1.getClassState(foreignSchema), ref).markAsFromDatabase();
        return ref;
    }
    hydrateModel(model, classSchema, dbRecord) {
        ...
        const classState = classSchema === this.rootClassSchema ? this.rootClassState : identity_map_1.getClassState(classSchema);
        ...
        if (this.rootClassSchema.references.size > 0) {
            ...
            pkHash = classSchema === this.rootClassSchema ? this.rootPkHash(dbRecord) : type_1.getPrimaryKeyHashGenerator(classSchema, this.serializer)(dbRecord);
        ...
        if (this.identityMap && !partial) {
            ...
            if (item) {
                const fromDatabase = identity_map_1.getInstanceState(classState, item).isFromDatabase();
                //if its proxy a unhydrated proxy then we update property values
                if (fromDatabase && !type_1.isReferenceHydrated(item)) {
        ...
        if (!partial) {
            identity_map_1.getInstanceState(classState, converted).markAsPersisted();
            if (pool)
    ...
    createObject(model, classState, classSchema, dbRecord) {
        ...
        if (!partial) {
            identity_map_1.getInstanceState(classState, converted).markAsFromDatabase();
        }
