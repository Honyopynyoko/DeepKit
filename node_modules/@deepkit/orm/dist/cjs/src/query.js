...
exports.JoinDatabaseQuery = exports.Query = exports.GenericQueryResolver = exports.BaseQuery = exports.ItemNotFound = exports.DatabaseQueryModel = void 0;
...
class DatabaseQueryModel {
    ...
    hasParameters() {
        return !core_1.empty(this.parameters);
    }
}
exports.DatabaseQueryModel = DatabaseQueryModel;
...
class Query extends BaseQuery {
    ...
    async callOnFetchEvent(query) {
        ...
        if (!hasEvents)
            return query;
        const event = new event_1.QueryDatabaseEvent(this.databaseSession, this.classSchema, query);
    ...
    onQueryResolve(query) {
        if (query.classSchema.singleTableInheritance && query.classSchema.superClass) {
            ...
            const value = query.classSchema.getProperty(discriminant.name).getDefaultValue() || type_1.getSingleTableInheritanceTypeValue(query.classSchema);
    ...
    async find() {
        ...
        const frame = this.databaseSession.stopwatch.start(this.classSchema.getClassName() + ': Find', stopwatch_1.FrameCategory.database);
        try {
            const query = this.onQueryResolve(await this.callOnFetchEvent(this));
            return await query.resolver.find(query.model);
        }
    ...
    async delete(query) {
        ...
        const event = new event_1.QueryDatabaseDeleteEvent(this.databaseSession, this.classSchema, query, deleteResult);
    ...
    async patch(query, patch) {
        ...
        const event = new event_1.QueryDatabasePatchEvent(this.databaseSession, this.classSchema, query, changes, patchResult);
        ...
        if (query.model.withIdentityMap) {
            const pkHashGenerator = type_1.getSimplePrimaryKeyHashGenerator(this.classSchema);
...
exports.Query = Query;
class JoinDatabaseQuery extends BaseQuery {
...
exports.JoinDatabaseQuery = JoinDatabaseQuery;
