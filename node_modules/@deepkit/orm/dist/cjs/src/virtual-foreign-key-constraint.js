...
exports.VirtualForeignKeyConstraint = void 0;
/**
 ...
 */
class VirtualForeignKeyConstraint {
    constructor(database) {
        this.database = database;
    }
    resolveReferencesTo(fromClassSchema) {
        ...
        for (const classSchema of this.database.entities.values()) {
            for (const reference of classSchema.references.values()) {
                if (reference.referenceOptions.onDelete === 'NO ACTION')
                    continue;
    ...
    async onQueryDelete(event) {
        const references = this.resolveReferencesTo(event.classSchema);
        if (!references.length)
            return;
        if (!event.deleteResult.primaryKeys.length)
            return;
    ...
    async onQueryPatch(event) {
        const references = this.resolveReferencesTo(event.classSchema);
        if (!references.length)
            return;
        if (!event.patchResult.primaryKeys.length)
            return;
        const primaryKeyName = event.classSchema.getPrimaryField().name;
        for (const { classSchema, property } of references) {
            if (!event.patch.has(property.name))
                continue;
...
exports.VirtualForeignKeyConstraint = VirtualForeignKeyConstraint;
//# sourceMappingURL=virtual-foreign-key-constraint.js.map