 ...
 */
import { changeSetSymbol, getChangeDetector, getClassSchema, getConverterForSnapshot, getPrimaryKeyExtractor, getPrimaryKeyHashGenerator, getSimplePrimaryKeyHashGenerator } from '@deepkit/type';
import { isObject, toFastProperties } from '@deepkit/core';
import { inspect } from 'util';
export function getNormalizedPrimaryKey(schema, primaryKey) {
    ...
    if (primaryFields.length > 1) {
        ...
        if (isObject(primaryKey) && primaryKey[first.name] !== undefined) {
...
export class ClassState {
    constructor(classSchema) {
        ...
        this.snapshot = getConverterForSnapshot(this.classSchema);
        this.primaryKeyExtractor = getPrimaryKeyExtractor(this.classSchema);
        this.primaryKeyHashGenerator = getPrimaryKeyHashGenerator(this.classSchema);
        this.simplePrimaryKeyHashGenerator = getSimplePrimaryKeyHashGenerator(this.classSchema);
        this.changeDetector = getChangeDetector(this.classSchema);
    }
}
export function getClassState(classSchema) {
...
class InstanceState {
    ...
    markAsPersisted() {
        ...
        this.snapshot = this.classState.snapshot(this.item);
        if (this.item[changeSetSymbol])
            this.item[changeSetSymbol].clear();
    }
    getLastKnownPK() {
...
export function getInstanceStateFromItem(item) {
    return getInstanceState(getClassState(getClassSchema(item)), item);
}
export function getInstanceState(classState, item) {
    ...
    if (!item['constructor'].prototype.hasOwnProperty(changeSetSymbol)) {
        Object.defineProperty(item['constructor'].prototype, changeSetSymbol, {
            writable: true,
            enumerable: false,
            value: null
        });
