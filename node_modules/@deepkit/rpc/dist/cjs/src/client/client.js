...
exports.RpcClient = exports.RpcBaseClient = exports.RpcClientPeer = exports.RpcClientTransporter = exports.RpcClientToken = exports.OfflineError = void 0;
const core_1 = require("@deepkit/core");
const rxjs_1 = require("rxjs");
const model_1 = require("../model");
const protocol_1 = require("../protocol");
const kernel_1 = require("../server/kernel");
const writer_1 = require("../writer");
const action_1 = require("./action");
const message_subject_1 = require("./message-subject");
...
exports.RpcClientToken = RpcClientToken;
class RpcClientTransporter {
    constructor(transport) {
        ...
        this.reconnected = new rxjs_1.Subject();
        this.disconnected = new rxjs_1.Subject();
        this.reader = new protocol_1.RpcMessageReader((v) => this.onMessage(v), (id) => {
            if (this.writer) {
            ...
            try {
                await this.transport.connect({
                    ...
                    onConnected: async (transport) => {
                        ...
                        this.writer = new writer_1.RpcMessageWriter({
                            write(v) {
                                transport.send(v);
                            },
                            close() {
...
exports.RpcClientTransporter = RpcClientTransporter;
class RpcClientPeer {
    ...
    controller(nameOrDefinition, options = {}) {
        const controller = new action_1.RpcControllerState('string' === typeof nameOrDefinition ? nameOrDefinition : nameOrDefinition.path);
...
exports.RpcClientPeer = RpcClientPeer;
class RpcBaseClient {
    constructor(transport) {
        this.transport = transport;
        this.messageId = 1;
        this.replies = new Map();
        this.actionClient = new action_1.RpcActionClient(this);
    ...
    async onAuthenticate() {
        if (!this.token.has())
            return;
        const reply = await this.sendMessage(model_1.RpcTypes.Authenticate, model_1.rpcAuthenticate, { token: this.token.get() }, { dontWaitForConnection: true })
            .waitNextMessage();
        if (reply.isError())
            throw reply.getError();
        if (reply.type === model_1.RpcTypes.AuthenticateResponse) {
            const body = reply.parseBody(model_1.rpcResponseAuthenticate);
    ...
    onMessage(message) {
        ...
        if (message.type === model_1.RpcTypes.Entity) {
    ...
    sendMessage(type, schema, body, options = {}) {
        ...
        const continuation = (type, schema, body) => {
            if (connectionId === this.transporter.connectionId) {
                ...
                const message = protocol_1.createRpcMessage(id, type, schema, body);
        ...
        if (dontWaitForConnection || this.transporter.isConnected()) {
            const message = options && options.peerId
                ? protocol_1.createRpcMessagePeer(id, type, this.getId(), options.peerId, schema, body)
                : protocol_1.createRpcMessage(id, type, schema, body);
            ...
            this.transporter.connect().then(() => {
                ...
                subject.next(new protocol_1.ErroredRpcMessage(id, e));
            });
        }
        return subject;
    }
...
class RpcClient extends RpcBaseClient {
    constructor() {
        super(...arguments);
    ...
    async onHandshake() {
        ...
        const reply = await this.sendMessage(model_1.RpcTypes.ClientId, undefined, undefined, { dontWaitForConnection: true })
            .firstThenClose(model_1.RpcTypes.ClientIdResponse, model_1.rpcClientId);
        return reply.id;
    }
    async ping() {
        await this.sendMessage(model_1.RpcTypes.Ping).waitNext(model_1.RpcTypes.Pong);
    }
    onMessage(message) {
        if (message.routeType === 3 /* peer */) {
            ...
            if (!connection) {
                ...
                if (!(c instanceof kernel_1.RpcKernelConnection))
    ...
    registerController(nameOrDefinition, classType) {
        if (!this.clientKernel)
            this.clientKernel = new kernel_1.RpcKernel();
    ...
    async registerAsPeer(id) {
        ...
        this.peerKernel = new kernel_1.RpcKernel();
        await this.sendMessage(model_1.RpcTypes.PeerRegister, model_1.rpcPeerRegister, { id }).firstThenClose(model_1.RpcTypes.Ack);
        this.registeredAsPeer = id;
        return {
            deregister: async () => {
                await this.sendMessage(model_1.RpcTypes.PeerDeregister, model_1.rpcPeerDeregister, { id }).firstThenClose(model_1.RpcTypes.Ack);
