...
exports.rpcDecodeError = exports.rpcEncodeError = exports.RpcBufferReader = exports.readUint32LE = exports.RpcMessageReader = exports.createRpcMessageSourceDestForBody = exports.createRpcMessageSourceDest = exports.createRpcMessagePeer = exports.createRpcMessageForBody = exports.createRpcMessage = exports.createRpcCompositeMessageSourceDest = exports.createRpcCompositeMessage = exports.createBuffer = exports.readRpcMessage = exports.ErroredRpcMessage = exports.RpcMessage = void 0;
...
class RpcMessage {
    ...
    getError() {
        if (!this.buffer)
            throw new Error('No buffer');
        const error = bson_1.getBSONDecoder(model_1.rpcError)(this.buffer, this.bodyOffset);
        return rpcDecodeError(error);
    }
    isError() {
        return this.type === model_1.RpcTypes.Error;
    }
    parseBody(schema) {
        if (!this.bodySize)
...
class ErroredRpcMessage extends RpcMessage {
    constructor(id, error) {
        super(id, false, model_1.RpcTypes.Error, 0, 0, 0);
        this.id = id;
        this.error = error;
    }
    getError() {
...
exports.ErroredRpcMessage = ErroredRpcMessage;
function readRpcMessage(buffer) {
    const view = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
...
exports.readRpcMessage = readRpcMessage;
exports.createBuffer = 'undefined' !== typeof Buffer && 'function' === typeof Buffer.allocUnsafe ? Buffer.allocUnsafe : (size) => new Uint8Array(size);
function createRpcCompositeMessage(id, type, messages, routeType = 0 /* client */) {
    let bodySize = 0;
    for (const message of messages) {
        bodySize += 4 + 1 + (message.schema && message.body ? bson_1.getBSONSizer(message.schema)(message.body) : 0);
    }
    //<size> <version> <messageId> <routeType>[routeData] <isComposite> <type> <body...>
    const messageSize = 4 + 1 + 4 + 1 + 1 + 1 + bodySize;
    const writer = new bson_1.Writer(exports.createBuffer(messageSize));
    ...
    for (const message of messages) {
        writer.writeUint32(message.schema && message.body ? bson_1.getBSONSizer(message.schema)(message.body) : 0);
        writer.writeByte(message.type); //type
        if (message.schema && message.body) {
            //BSON object contain already their size at the beginning
            bson_1.getBSONSerializer(message.schema)(message.body, writer);
        }
    }
    return writer.buffer;
}
exports.createRpcCompositeMessage = createRpcCompositeMessage;
function createRpcCompositeMessageSourceDest(id, source, destination, type, messages) {
    let bodySize = 0;
    for (const message of messages) {
        bodySize += 4 + 1 + (message.schema && message.body ? bson_1.getBSONSizer(message.schema)(message.body) : 0);
    }
...
exports.createRpcCompositeMessageSourceDest = createRpcCompositeMessageSourceDest;
function createRpcMessage(id, type, schema, body, routeType = 0 /* client */) {
    if (schema) {
        if (!schema.jit.bsonSizer)
            bson_1.getBSONSizer(schema);
        if (!schema.jit.bsonSerializer)
            bson_1.getBSONSerializer(schema);
    }
...
function createRpcMessageForBody(id, type, body, routeType = 0 /* client */) {
...
exports.createRpcMessageForBody = createRpcMessageForBody;
function createRpcMessagePeer(id, type, source, peerId, schema, body) {
    const bodySize = schema && body ? bson_1.getBSONSizer(schema)(body) : 0;
    //<size> <version> <messageId> <routeType>[routeData] <composite> <type> <body...>
    ...
    writer.writeByte(type);
    if (schema && body)
        bson_1.getBSONSerializer(schema)(body, writer);
    return writer.buffer;
}
exports.createRpcMessagePeer = createRpcMessagePeer;
function createRpcMessageSourceDest(id, type, source, destination, schema, body) {
    const bodySize = schema && body ? bson_1.getBSONSizer(schema)(body) : 0;
    //<size> <version> <messageId> <routeType>[routeData] <composite> <type> <body...>
    const messageSize = 4 + 1 + 4 + 1 + (16 + 16) + 1 + 1 + bodySize;
    const writer = new bson_1.Writer(exports.createBuffer(messageSize));
...
exports.createRpcMessageSourceDest = createRpcMessageSourceDest;
function createRpcMessageSourceDestForBody(id, type, source, destination, body) {
    //<size> <version> <messageId> <routeType>[routeData] <composite> <type> <body...>
...
exports.createRpcMessageSourceDestForBody = createRpcMessageSourceDestForBody;
class RpcMessageReader {
    ...
    gotMessage(buffer) {
        ...
        if (message.type === model_1.RpcTypes.ChunkAck) {
            const ack = this.chunkAcks.get(message.id);
            if (ack)
                ack();
        }
        else if (message.type === model_1.RpcTypes.Chunk) {
            const progress = this.progress.get(message.id);
            const body = message.parseBody(model_1.rpcChunk);
            ...
            if (chunks.loaded === body.total) {
                ...
                let offset = 0;
                const newBuffer = exports.createBuffer(body.total);
...
exports.RpcMessageReader = RpcMessageReader;
...
exports.RpcBufferReader = RpcBufferReader;
function rpcEncodeError(error) {
    let classType = '';
    let stack = '';
    let properties;
    if ('string' !== typeof error) {
        const schema = type_1.getClassSchema(error['constructor']);
        stack = error.stack || '';
        if (schema.name) {
            classType = schema.name;
            if (schema.getPropertiesMap().size) {
                properties = type_1.jsonSerializer.for(schema).serialize(error);
            }
        }
    }
    return {
...
exports.rpcEncodeError = rpcEncodeError;
function rpcDecodeError(error) {
    if (error.classType) {
        const entity = type_1.getGlobalStore().RegisteredEntities[error.classType];
        if (!entity) {
        ...
        const classType = type_1.getClassSchema(entity).classType;
        if (error.properties) {
            const e = type_1.jsonSerializer.for(type_1.getClassSchema(entity)).deserialize(error.properties);
            e.stack = error.stack + '\nat ___SERVER___';
            return e;
        }
...
exports.rpcDecodeError = rpcDecodeError;
