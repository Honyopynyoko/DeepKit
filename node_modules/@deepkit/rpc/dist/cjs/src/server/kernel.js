...
class RpcKernelBaseConnection {
    constructor(transportWriter, connections) {
        ...
        this.writer = new writer_1.RpcMessageWriter(this.transportWriter, this.reader);
        this.timeoutTimers = [];
...
class RpcKernel {
     ...
     */
    registerController(id, controller, addAsProvider = true) {
        if (addAsProvider) {
            if (this.injector instanceof injector_1.InjectorContext) {
                this.injector.contextManager.get(0).providers.push({ provide: controller, scope: 'rpc' });
            }
            if (this.injector instanceof injector_1.Injector) {
    ...
    createConnection(writer, injector) {
        let connection;
        if (!injector) {
            const subInjector = new injector_1.MemoryInjector([
                { provide: RpcKernelConnection, useFactory: () => connection },
                { provide: security_1.SessionState, useFactory: () => connection.sessionState },
            ]);
            let parent = this.injector;
            if (parent instanceof injector_1.InjectorContext) {
                parent = parent.createChildScope('rpc');
            }
            const childInjectors = [subInjector, parent];
            injector = new injector_1.Injector([], childInjectors);
        }
