 ...
 */
import { collectForMicrotask, getClassName, getClassPropertyName, isArray, isPlainObject, isPrototypeOfBase, stringifyValueWithType, toFastProperties } from '@deepkit/core';
import { isBehaviorSubject, isSubject } from '@deepkit/core-rxjs';
import { createClassSchema, getClassSchema, getXToClassFunction, jitValidate, jsonSerializer, propertyDefinition, PropertySchema, t } from '@deepkit/type';
import { isObservable, Observable } from 'rxjs';
import { Collection, CollectionQueryModel, CollectionState, isCollection } from '../collection';
import { getActionParameters, getActions } from '../decorators';
import { ActionObservableTypes, EntitySubject, isEntitySubject, rpcActionObservableSubscribeId, rpcActionType, rpcResponseActionCollectionRemove, rpcResponseActionCollectionSort, rpcResponseActionObservable, rpcResponseActionObservableSubscriptionError, rpcResponseActionType, RpcTypes, ValidationError } from '../model';
import { rpcEncodeError } from '../protocol';
    ...
    async handleAction(message, response) {
        ...
        try {
            ...
            if (isEntitySubject(result)) {
                ...
                const eventsSub = collection.event.subscribe(collectForMicrotask((events) => {
                    if (unsubscribed)
                        return;
                ...
                let type = ActionObservableTypes.observable;
                if (isSubject(result)) {
                    ...
                    this.observableSubjects[message.id] = {
                        ...
                        subscription: result.subscribe((next) => {
                            if (types.resultProperty.type === 'class' && types.resultProperty.classType && next && !(next instanceof types.resultProperty.classType)) {
                                console.warn(`The subject in action ${getClassPropertyName(classType, body.method)} has a class type assigned of ${getClassName(types.resultProperty.classType)}` +
...
export function createNewPropertySchemaIfNecessary(result, property) {
...
export function isResultTypeDifferent(result, property) {
    ...
    if (property.type === 'arrayBuffer' && !(result instanceof ArrayBuffer))
        return true;
    if (property.type === 'map' && !isPlainObject(result))
        return true;
    if (property.type === 'array' && !isArray(result))
        return true;
