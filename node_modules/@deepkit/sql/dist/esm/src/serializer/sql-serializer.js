...
import { getClassSchema, getDataConverterJS, jsonSerializer, nodeBufferToArrayBuffer, nodeBufferToTypedArray, typedArrayNamesMap, typedArrayToBuffer } from '@deepkit/type';
...
export function uuid4Binary(u) {
    return 'string' === typeof u ? Buffer.from(u.replace(/-/g, ''), 'hex') : Buffer.alloc(0);
}
export function uuid4Stringify(buffer) {
...
sqlSerializer.fromClass.register('undefined', (property, state) => {
    //sql does not support 'undefined' as column type, so we convert automatically to null
    state.addSetter(`null`);
});
sqlSerializer.fromClass.register('null', (property, state) => {
    //sql does not support 'undefined' as column type, so we convert automatically to null
    state.addSetter(`null`);
});
sqlSerializer.toClass.register('null', (property, state) => {
...
sqlSerializer.toClass.register('undefined', (property, state) => {
    //sql does not support 'undefined' as column type, so we store always null. depending on the property definition
    //we convert back to undefined or keep it null
    if (property.isOptional)
        return state.addSetter(`undefined`);
...
sqlSerializer.fromClass.register('date', (property, state) => {
    state.addSetter(`${state.accessor}`);
});
sqlSerializer.toClass.register('uuid', (property, state) => {
...
sqlSerializer.fromClass.register('uuid', (property, state) => {
...
sqlSerializer.toClass.prepend('class', (property, state) => {
...
sqlSerializer.fromClass.prepend('class', (property, state) => {
...
sqlSerializer.fromClass.append('class', (property, state) => {
    if (property.isReference)
        return;
...
sqlSerializer.fromClass.register('array', (property, state) => {
    if (property.isReference)
        return;
    //we don't stringify non-root properties
    if (property.parent)
...
sqlSerializer.toClass.prepend('array', (property, state) => {
    if (property.parent)
        return;
    state.addSetter(`'string' === typeof ${state.accessor} ? JSON.parse(${state.accessor}) : ${state.accessor}`);
});
sqlSerializer.fromClass.append('map', (property, state) => {
    //we don't stringify non-root properties
    if (property.parent)
        return;
    //we need to convert the structure to JSON-string after it has been converted to JSON values from the previous compiler
...
sqlSerializer.toClass.prepend('map', (property, state) => {
    if (property.parent)
        return;
    state.addSetter(`'string' === typeof ${state.accessor} ? JSON.parse(${state.accessor}) : ${state.accessor}`);
});
sqlSerializer.fromClass.registerForBinary((property, state) => {
    state.setContext({ typedArrayToBuffer });
    state.addSetter(`typedArrayToBuffer(${state.accessor})`);
});
sqlSerializer.toClass.registerForBinary((property, state) => {
...
sqlSerializer.toClass.register('arrayBuffer', (property, state) => {
