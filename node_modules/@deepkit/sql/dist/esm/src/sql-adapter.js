 ...
 */
import { DatabaseAdapter, DatabaseAdapterQueryFactory, DatabaseError, DatabaseLogger, DatabasePersistence, DatabaseQueryModel, GenericQueryResolver, Query } from '@deepkit/orm';
import { isArray } from '@deepkit/core';
import { ClassSchema, getClassSchema, hasClassSchema, plainToClass, t } from '@deepkit/type';
...
export class SQLQueryModel extends DatabaseQueryModel {
    clone(parentQuery) {
...
export class SQLConnection {
    constructor(connectionPool, logger = new DatabaseLogger) {
...
export class SQLQueryResolver extends GenericQueryResolver {
    ...
    createFormatter(withIdentityMap = false) {
        return new SqlFormatter(this.classSchema, sqlSerializer, this.session.getHydrator(), withIdentityMap ? this.session.identityMap : undefined);
    }
...
export class SQLDatabaseQuery extends Query {
...
export class SQLDatabaseQueryFactory extends DatabaseAdapterQueryFactory {
    ...
    createQuery(classType) {
        return new SQLDatabaseQuery(getClassSchema(classType), this.databaseSession, new SQLQueryResolver(this.connectionPool, this.platform, getClassSchema(classType), this.databaseSession));
    }
}
...
export class SQLDatabaseAdapter extends DatabaseAdapter {
    rawFactory(session) {
