...
exports.SQLiteSchemaParser = void 0;
const sql_1 = require("@deepkit/sql");
const core_1 = require("@deepkit/core");
class SQLiteSchemaParser extends sql_1.SchemaParser {
    async parse(database, limitTableNames) {
    ...
    async addIndexes(database, table) {
        ...
        for (const row of rows) {
            ...
            for (const indexRow of indexInfo)
                index.addColumn(indexRow.name);
            if (index.columns.length === 1 && table.hasPrimaryKey() && table.getPrimaryKeys()[0].name === index.columns[0].name) {
                ...
                core_1.arrayRemoveItem(table.indices, index);
            }
        }
    }
    async addForeignKeys(database, table) {
        const tableName = table.getFullName(this.platform.getSchemaDelimiter());
        ...
        for (const row of rows) {
            if (lastId !== row.id) {
                ...
                if (row.on_update)
                    fk.onUpdate = row.on_update;
                if (row.on_delete)
                    fk.onDelete = row.on_delete;
            }
            if (fk)
                fk.addReference(row.from, row.to);
        }
    }
    async addColumns(table) {
        const tableName = table.getFullName(this.platform.getSchemaDelimiter());
        ...
        for (const row of rows) {
            ...
            sql_1.parseType(column, fullType);
            ...
            if (column.isPrimaryKey) {
                ...
                if (aiRow && aiRow.tbl_name === tableName)
                    column.isAutoIncrement = true;
            }
        }
    }
    async parseTables(database, limitTableNames) {
        let filter = '';
        ...
        for (const row of rows) {
            ...
            if (limitTableNames && !limitTableNames.includes(tableName))
                continue;
            if (this.platform.getMigrationTableName() === tableName)
                continue;
...
//# sourceMappingURL=sqlite-schema-parser.js.map