...
exports.convertJsxCodeToCreateElement = exports.optimizeJSX = exports.transform = void 0;
const pirates_1 = require("pirates");
// @ts-ignore
const abstract_syntax_tree_1 = __importDefault(require("abstract-syntax-tree"));
const core_1 = require("@deepkit/core");
const { parse, generate, replace } = abstract_syntax_tree_1.default;
function transform(code, filename) {
    ...
    if (code.indexOf('.jsx(') === -1 && code.indexOf('.jsxs(') === -1)
        return code;
    return optimizeJSX(code);
}
exports.transform = transform;
pirates_1.addHook(transform, { exts: ['.js', '.tsx'] });
...
function serializeValue(node) {
    ...
    if (node.type === 'UnaryExpression' && ((_a = node.argument) === null || _a === void 0 ? void 0 : _a.type) === 'Literal') {
        return node.argument.value;
    }
    return NotSerializable;
}
function optimizeAttributes(node) {
    let value = [];
    for (const p of node.properties) {
        ...
        if (!keyName)
            return node;
        if (p.value.type !== 'Literal' && p.value.type !== 'UnaryExpression')
            return node;
        value.push(keyName + '="' + serializeValue(p.value) + '"');
    }
    return { type: 'Literal', value: value.join(' ') };
}
function isCjsJSXCall(node) {
    var _a, _b;
    return node.type === 'CallExpression' && (((_a = node.callee.property) === null || _a === void 0 ? void 0 : _a.name) === 'jsx' || ((_b = node.callee.property) === null || _b === void 0 ? void 0 : _b.name) === 'jsxs');
}
function isESMJSXCall(node) {
...
function optimizeNode(node) {
    ...
    if (node.callee.object.type !== 'Identifier')
        return node;
    const isCreateElementExpression = node.callee.property.type === 'Identifier' && ((_a = node.callee.property) === null || _a === void 0 ? void 0 : _a.name) === 'createElement';
    if (!isCreateElementExpression)
        return node;
...
function extractChildrenFromObjectExpressionProperties(props) {
    ...
    if (node.arguments[1].type === 'CallExpression' && node.arguments[1].callee.type === 'MemberExpression' && node.arguments[1].callee.object.type === 'Identifier' && node.arguments[1].callee.object.name === 'Object') {
        ...
        if (children) {
            if (children.type === 'ArrayExpression') {
                node.arguments.push(...children.elements.map(v => convertNodeToCreateElement(v)));
            }
            else {
...
exports.optimizeJSX = optimizeJSX;
function convertJsxCodeToCreateElement(code) {
    const tree = parse(code);
    replace(tree, (node) => {
        if (isESMJSXCall(node) || isCjsJSXCall(node)) {
            try {
...
exports.convertJsxCodeToCreateElement = convertJsxCodeToCreateElement;
//# sourceMappingURL=optimize-tsx.js.map