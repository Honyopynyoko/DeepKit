...
exports.applyPatch = exports.applyAndReturnPatches = exports.Patcher = exports.shallowCloneObject = void 0;
 ...
 */
function shallowCloneObject(item) {
    if (model_1.isClassInstance(item)) {
        const classType = model_1.getClassTypeFromInstance(item);
...
class Patcher {
    ...
    getArrayProxy(originalArray, dereferenceArray) {
        let dereferenced;
        return new Proxy([], {
            get: (target, prop) => {
                if (prop === 'splice' || prop === 'push' || prop === 'pop' || prop === 'unshift' || prop === 'shift') {
                    if (!dereferenced)
                        dereferenced = dereferenceArray();
                }
    ...
    getProxy(incomingPath, originalItem, dereferenceOriginalItem) {
        let proxy = this.proxies.get(originalItem);
        if (proxy)
            return proxy;
        const dereferencedObjects = new Map();
        const dereferencedArrays = new Map();
        let dereferencedOriginalItem = undefined;
        function dereferenceOriginalItemAndCache() {
            dereferencedOriginalItem = dereferenceOriginalItem();
            return dereferencedOriginalItem;
        }
        const dereferenceObject = (path = '') => {
        ...
        proxy = new Proxy({}, {
            get: (target, prop) => {
                ...
                if (core_1.isObject(originalItem[prop])) {
                ...
                if (core_1.isArray(originalItem[prop])) {
            ...
            set: (target, prop, value) => {
                ...
                if (value !== undefined && value === state[prop])
                    return true;
                const fullPath = incomingPath ? incomingPath + '.' + String(prop) : String(prop);
 ...
 */
function applyAndReturnPatches(item, patch) {
    const patcher = new Patcher(item);
    patch(patcher.proxy);
    return patcher.patches;
}
exports.applyAndReturnPatches = applyAndReturnPatches;
/**
