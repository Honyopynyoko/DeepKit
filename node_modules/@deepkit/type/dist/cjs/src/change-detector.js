...
exports.buildChanges = exports.getChangeDetector = void 0;
...
function createJITChangeDetectorForSnapshot(schema, jitStack = new jit_1.JitStack()) {
    const context = new Map();
    const prepared = jitStack.prepare(schema);
    context.set('genericEqual', genericEqual);
    context.set('empty', core_1.empty);
    const props = [];
    ...
    function getComparator(property, last, current, accessor, changedName, onChanged, jitStack) {
        if (property.isArray) {
            const l = serializer_compiler_1.reserveVariable(context, 'l');
            return `
                if (!${has(changedName)}) {
                if (!${current} && !${last}) {
            ...
            context.set('getObjectKeysSize', core_1.getObjectKeysSize);
            const i = serializer_compiler_1.reserveVariable(context, 'i');
            return `
                if (!${has(changedName)}) {
                if (!${current} && !${last}) {
            ...
            const jitChangeDetectorThis = serializer_compiler_1.reserveVariable(context, 'jitChangeDetector');
    ...
    context.set('changeSetSymbol', changes_1.changeSetSymbol);
    context.set('ItemChanges', changes_1.ItemChanges);
    const functionCode = `
...
function getChangeDetector(classSchema) {
    return classSchema.getJit(changeDetectorSymbol, () => createJITChangeDetectorForSnapshot(classSchema));
}
exports.getChangeDetector = getChangeDetector;
function buildChanges(classSchema, lastSnapshot, item) {
    const currentSnapshot = snapshot_1.getConverterForSnapshot(classSchema)(item);
    return getChangeDetector(classSchema)(lastSnapshot, currentSnapshot, item) || new changes_1.Changes();
}
