...
exports.MultiIndex = exports.t = exports.type = exports.field = exports.f = exports.OnLoad = exports.ParentReference = exports.FieldDecoratorWrapper = exports.createValidatorFromFunction = exports.DatabaseName = exports.entity = exports.Entity = void 0;
const core_1 = require("@deepkit/core");
const utils_1 = require("./utils");
const decorator_builder_1 = require("./decorator-builder");
 ...
 * @category Decorator
 */
function Entity(name, collectionName) {
    return (target) => {
        if (model_1.getGlobalStore().RegisteredEntities[name]) {
        ...
        model_1.getGlobalStore().RegisteredEntities[name] = target;
        model_1.getOrCreateEntitySchema(target).name = name;
        model_1.getOrCreateEntitySchema(target).collectionName = collectionName;
    };
}
exports.Entity = Entity;
class EntityApi {
    constructor(target) {
        this.t = new model_1.ClassSchema(class {
        });
        this.t = model_1.getClassSchema(target);
    }
    name(name) {
        this.t.name = name;
        if (model_1.getGlobalStore().RegisteredEntities[name]) {
        ...
        model_1.getGlobalStore().RegisteredEntities[name] = this.t;
        this.t.name = name;
    }
...
exports.entity = decorator_builder_1.createClassDecoratorContext(EntityApi);
/**
...
function DatabaseName(name) {
    return (target) => {
        model_1.getOrCreateEntitySchema(target).databaseSchemaName = name;
    };
}
exports.DatabaseName = DatabaseName;
function createValidatorFromFunction(validator) {
    return class {
...
exports.createValidatorFromFunction = createValidatorFromFunction;
function createFieldDecoratorResult(cb, givenPropertyName = '', modifier = [], root = false) {
    ...
    function buildPropertySchema(target, propertyOrMethodName, parameterIndexOrDescriptor, parent) {
        //anon properties
        const propertySchema = new model_1.PropertySchema(propertyOrMethodName || String(parameterIndexOrDescriptor), parent);
        for (const mod of modifier) {
            mod(target, propertySchema);
        }
        if (core_1.isNumber(parameterIndexOrDescriptor) && target['prototype']) {
    ...
    const fn = (target, propertyOrMethodName, parameterIndexOrDescriptor) => {
        ...
        const schema = model_1.getOrCreateEntitySchema(target);
        const isMethod = propertyOrMethodName && (Reflect.hasMetadata && Reflect.hasMetadata('design:returntype', target, propertyOrMethodName)) && !core_1.isNumber(parameterIndexOrDescriptor);
        if (core_1.isNumber(parameterIndexOrDescriptor)) {
            ...
            if (givenPropertyName && ((methodsParamNames[parameterIndexOrDescriptor] && methodsParamNames[parameterIndexOrDescriptor] !== givenPropertyName)
                ...
                throw new Error(`Defining multiple deepkit/type decorators with different names at arguments of ${core_1.getClassName(target)}::${methodName} #${parameterIndexOrDescriptor} is forbidden.` +
            ...
            if (givenPropertyName) {
                ...
                const constructorParamNames = core_1.extractParameters(target.prototype.constructor);
        ...
        if (isMethod && propertyOrMethodName) {
            ...
            if (!schema.methods[propertyOrMethodName]) {
                schema.methods[propertyOrMethodName] = new model_1.PropertySchema(propertyOrMethodName);
            }
            ...
            if (core_1.isNumber(parameterIndexOrDescriptor)) {
                ...
                if (methodName === 'constructor') {
                    if (!schema.getPropertiesMap().has(givenPropertyName)) {
                        const property = new model_1.PropertySchema(givenPropertyName);
                    ...
                    if (!argumentsProperties[parameterIndexOrDescriptor]) {
                        ...
                        if (!method) {
                            throw new Error(`Decorator @t used on a methods argument where the method does not exist. `
                                ...
                                + `${core_1.getClassPropertyName(target, methodName)} #${parameterIndexOrDescriptor}`);
                        }
                        const constructorParamNames = core_1.extractParameters(target[methodName]);
                        const name = constructorParamNames[parameterIndexOrDescriptor] || String(parameterIndexOrDescriptor);
                        argumentsProperties[parameterIndexOrDescriptor] = new model_1.PropertySchema(name);
                ...
                if (!givenPropertyName) {
                    throw new Error(`Could not resolve property name for class property on ${core_1.getClassName(target)}`);
                }
                propertySchema = schema.getPropertiesMap().get(givenPropertyName) || new model_1.PropertySchema(givenPropertyName);
    ...
    for (const [validatorName, validatorFn] of Object.entries(validation_decorator_1.validators)) {
    ...
    fn.reference = (options) => {
        ...
        return createFieldDecoratorResult(cb, givenPropertyName, [...modifier, (target, property) => {
                ...
                model_1.getClassSchema(target).registerReference(property);
            }]);
    };
    ...
    fn.generic = fn.template = (...templateArgs) => {
        ...
        return createFieldDecoratorResult(cb, givenPropertyName, [...modifier, (target, property) => {
                ...
                for (const [i, t] of core_1.eachPair(templateArgs)) {
    ...
    fn.validator = (...validators) => {
        ...
        for (const validator of validators) {
            if (model_1.isPropertyValidator(validator)) {
 ...
 */
function FieldDecoratorWrapper(cb, root = false) {
    return createFieldDecoratorResult(cb, '', [], root);
}
exports.FieldDecoratorWrapper = FieldDecoratorWrapper;
/**
 * @internal
 */
function Decorated() {
    return (target, property) => {
        model_1.getOrCreateEntitySchema(target).decorator = property.name;
...
function Default(v) {
    return (target, property) => {
        if (utils_1.isArray(v)) {
            property.defaultValue = () => v.slice(0);
        }
        else if (core_1.isPlainObject(v)) {
            property.defaultValue = () => {
                return { ...v };
            };
        }
        else {
            property.defaultValue = core_1.isFunction(v) ? v : () => v;
        }
    };
}
/**
...
function Discriminant() {
    return (target, property) => {
        model_1.getOrCreateEntitySchema(target).discriminant = property.name;
...
function OnLoad(options = {}) {
    return (target, property) => {
        model_1.getOrCreateEntitySchema(target).onLoad.push({
            methodName: property,
            options: options,
        });
    };
    ...
    return FieldDecoratorWrapper((target, property, returnType) => {
        ...
        if (property.type === 'any' && !property.typeSet) {
            if (type) {
                ...
                else if (type instanceof model_1.ClassSchema) {
                    property.type = 'class';
                    property.classType = type.classType;
                    property.typeSet = true;
                }
                else if (core_1.isPlainObject(type)) {
                    property.type = 'class';
        ...
        const id = core_1.getClassName(target) + (property.methodName ? '::' + property.methodName : '') + '::' + propertyName;
...
fRaw['schema'] = function (props, options = {}, base) {
    ...
    let extendClazz;
    if (base) {
        if (base instanceof model_1.ClassSchema) {
    ...
    const schema = model_1.createClassSchema(clazz, options.name);
    ...
    for (const [name, prop] of Object.entries(props)) {
        ...
        else if (field_decorator_1.isFieldDecorator(prop)) {
            schema.addProperty(name, prop);
        }
        else if (prop instanceof model_1.ClassSchema) {
...
fRaw['partial'] = function (type) {
    let schema = type instanceof model_1.ClassSchema ? type.clone() : new model_1.ClassSchema(class {
    });
    if (core_1.isPlainObject(type)) {
        schema = exports.t.schema(type);
    }
    else {
        schema = model_1.getClassSchema(type).clone();
    }
 ...
 * @category Decorator
 */
function MultiIndex(fields, options, name) {
    return (target, property, parameterIndexOrDescriptor) => {
        const classType = target.prototype;
        const schema = model_1.getOrCreateEntitySchema(classType);
