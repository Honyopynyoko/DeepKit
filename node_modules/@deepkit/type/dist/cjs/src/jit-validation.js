...
exports.jitValidatePartial = exports.jitValidate = exports.jitValidateProperty = exports.getDataCheckerJS = exports.handleCustomValidator = exports.PropertyValidatorError = exports.ValidationFailedItem = void 0;
...
const jitFunctions = new WeakMap();
const CacheJitPropertyMap = new Map();
const CacheValidatorInstances = new Map();
/**
...
exports.ValidationFailedItem = ValidationFailedItem;
...
function handleCustomValidator(propSchema, validator, value, propertyPath, errors, classType) {
    try {
...
function getDataCheckerJS(path, accessor, property, rootContext, jitStack) {
    let compiler = jit_validation_registry_1.validationRegistry.get(property.type);
    if (property.noValidation)
        return '';
    ...
    function getCustomValidatorCode(accessor, path) {
        if (!property.validators.length)
            return '';
        const propertySchemaVar = serializer_compiler_1.reserveVariable(rootContext, 'schema_' + property.name);
        ...
        for (const validator of property.validators) {
            ...
            const validatorsVar = serializer_compiler_1.reserveVariable(rootContext, 'validator');
    ...
    if (property.isArray) {
        ...
        const varClassType = serializer_compiler_1.reserveVariable(rootContext);
        ...
        if (${accessor} === unpopulatedSymbol) {
            ...
            if (${accessor} === undefined) {
                ...
                ${jit_validation_registry_1.executeCheckerCompiler(path, rootContext, jitStack, compiler, accessor, property)}
            }
...
exports.getDataCheckerJS = getDataCheckerJS;
function jitValidateProperty(property, classType) {
    if (property.type === 'class') {
        const foreignSchema = model_1.getClassSchema(property.resolveClassType);
    ...
    const jitStack = new jit_1.JitStack();
    context.set('_classType', classType);
    context.set('ValidationError', ValidationFailedItem);
    context.set('_globalStore', model_1.getGlobalStore());
    context.set('ReturnSymbol', model_1.UnpopulatedCheck.ReturnSymbol);
    context.set('unpopulatedSymbol', model_1.unpopulatedSymbol);
    const functionCode = `
...
exports.jitValidateProperty = jitValidateProperty;
function jitValidate(schema, jitStack = new jit_1.JitStack()) {
    schema = schema instanceof model_1.ClassSchema ? schema : model_1.getClassSchema(schema);
    const jit = jitFunctions.get(schema);
    if (jit && jit.buildId === schema.buildId)
        return jit;
    const context = new Map();
    ...
    context.set('ReturnSymbol', model_1.UnpopulatedCheck.ReturnSymbol);
    context.set('unpopulatedSymbol', model_1.unpopulatedSymbol);
    context.set('isPlainObject', core_1.isPlainObject);
...
exports.jitValidate = jitValidate;
function jitValidatePartial(classType, partial, path, errors) {
    ...
    for (const i in partial) {
        ...
        jitValidateProperty(schema.getPropertiesMap().get(i) || jit_1.resolvePropertySchema(schema, i), classType)(partial[i], '', errors, thisPath);
    }
    return errors;
}
exports.jitValidatePartial = jitValidatePartial;
//# sourceMappingURL=jit-validation.js.map