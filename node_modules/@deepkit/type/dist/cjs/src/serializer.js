...
exports.emptySerializer = exports.ScopedSerializer = exports.Serializer = exports.SerializerCompilers = void 0;
const model_1 = require("./model");
const jit_1 = require("./jit");
const validation_1 = require("./validation");
const types_1 = require("./types");
class SerializerCompilers {
    ...
    registerForBinary(compiler) {
        for (const type of types_1.binaryTypes)
...
exports.SerializerCompilers = SerializerCompilers;
class Serializer {
    constructor(name) {
        this.name = name;
        /**
    ...
    for(schemaOrType) {
        return new ScopedSerializer(this, model_1.getClassSchema(schemaOrType));
    }
    /**
     * Serializes given class instance value to the serializer format.
     */
    serializeProperty(property, value) {
        return jit_1.getPropertyClassToXFunction(property, this)(value);
    }
    /**
     * Converts serialized value to class type.
     */
    deserializeProperty(property, value) {
        return jit_1.getPropertyXtoClassFunction(property, this)(value);
    }
    /**
...
class ScopedSerializer {
    ...
    serialize(instance, options) {
        if (!this._serialize)
            this._serialize = jit_1.getClassToXFunction(this.schema, this.serializer);
    ...
    validatedDeserialize(data, options) {
        if (!this._deserialize)
            this._deserialize = jit_1.getXToClassFunction(this.schema, this.serializer);
        const item = this._deserialize(data, options);
        const errors = validation_1.validate(this.schema, item);
        if (errors.length)
    ...
    serializeProperty(name, value) {
        var _a;
        const property = (_a = this.schema.getPropertiesMap().get(name)) !== null && _a !== void 0 ? _a : jit_1.resolvePropertySchema(this.schema, name);
        return jit_1.getPropertyClassToXFunction(property, this.serializer)(value);
    }
    /**
     * Converts given data in form of this serialization format to the target (default JS primitive/class) type.
     *
    ...
    deserializeProperty(name, value) {
        var _a;
        const property = (_a = this.schema.getPropertiesMap().get(name)) !== null && _a !== void 0 ? _a : jit_1.resolvePropertySchema(this.schema, name);
        return jit_1.getPropertyXtoClassFunction(property, this.serializer)(value);
    }
    serializeMethodArgument(methodName, property, value) {
        return jit_1.getPropertyClassToXFunction(this.schema.getMethodProperties(methodName)[property], this.serializer)(value);
    }
    deserializeMethodArgument(methodName, property, value) {
        return jit_1.getPropertyXtoClassFunction(this.schema.getMethodProperties(methodName)[property], this.serializer)(value);
    }
    serializeMethodResult(methodName, value) {
        return jit_1.getPropertyClassToXFunction(this.schema.getMethod(methodName), this.serializer)(value);
    }
    deserializeMethodResult(methodName, value) {
        return jit_1.getPropertyXtoClassFunction(this.schema.getMethod(methodName), this.serializer)(value);
    }
    /**
    ...
    partialSerialize(data, options) {
        if (!this._partialSerialize)
            this._partialSerialize = jit_1.getPartialClassToXFunction(this.schema, this.serializer);
    ...
    partialDeserialize(data, options) {
        if (!this._partialDeserialize)
            this._partialDeserialize = jit_1.getPartialXToClassFunction(this.schema, this.serializer);
    ...
    patchDeserialize(partial, options) {
        ...
        for (const i in partial) {
            if (!partial.hasOwnProperty(i))
                continue;
            const property = (_a = this.schema.getPropertiesMap().get(i)) !== null && _a !== void 0 ? _a : jit_1.resolvePropertySchema(this.schema, i);
            result[i] = jit_1.getPropertyXtoClassFunction(property, this.serializer)(partial[i], (_b = options === null || options === void 0 ? void 0 : options.parents) !== null && _b !== void 0 ? _b : [], options);
        }
        return result;
    }
    patchSerialize(partial, options) {
        ...
        for (const i in partial) {
            if (!partial.hasOwnProperty(i))
                continue;
            const property = (_a = this.schema.getPropertiesMap().get(i)) !== null && _a !== void 0 ? _a : jit_1.resolvePropertySchema(this.schema, i);
            result[i] = jit_1.getPropertyClassToXFunction(property, this.serializer)(partial[i], options);
        }
        return result;
    }
}
exports.ScopedSerializer = ScopedSerializer;
exports.emptySerializer = new class extends Serializer {
    constructor() {
        super('empty');
    }
};
