...
import { ClassSchema, getClassSchema, getGlobalStore, UnpopulatedCheck, unpopulatedSymbol } from './model';
import { executeCheckerCompiler, validationRegistry } from './jit-validation-registry';
...
export function handleCustomValidator(propSchema, validator, value, propertyPath, errors, classType) {
    try {
...
export function getDataCheckerJS(path, accessor, property, rootContext, jitStack) {
...
export function jitValidateProperty(property, classType) {
...
export function jitValidate(schema, jitStack = new JitStack()) {
...
export function jitValidatePartial(classType, partial, path, errors) {
    ...
    for (const i in partial) {
        ...
        jitValidateProperty(schema.getPropertiesMap().get(i) || resolvePropertySchema(schema, i), classType)(partial[i], '', errors, thisPath);
    }
    return errors;
}
