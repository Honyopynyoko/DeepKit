 ...
 */
import { getClassSchema, getGlobalStore, UnpopulatedCheck } from './model';
 ...
 * @hidden
 */
export function findParent(parents, parentType) {
    if (!parents)
        return;
 ...
 */
export function resolvePropertySchema(schema, propertyPath) {
    ...
    for (let i = 0; i < names.length; i++) {
        ...
        if (prop.isMap || prop.isArray) {
            if (prop.getSubType().type === 'class') {
                if (names[i + 2]) {
                    return cache[propertyPath] = resolvePropertySchema(getClassSchema(prop.getSubType().getResolvedClassType()), names.slice(i + 2).join('.'));
                }
                else if (names[i + 1]) {
...
export function getPropertyClassToXFunction(property, serializer) {
...
export function getPropertyXtoClassFunction(property, serializer) {
    const jit = property.jit;
 ...
 */
export function createPropertyClassToXFunction(property, serializer) {
 ...
 * Creates a new JIT compiled function to convert given property schema. Deep paths are not allowed.
 */
export function createPropertyXToClassFunction(property, serializer) {
    const context = new Map();
    const jitStack = new JitStack();
...
export function getParentResolverJS(schema, setter, property, context) {
...
export function createClassToXFunction(schema, serializer, jitStack = new JitStack()) {
    const context = new Map();
...
export function getClassToXFunction(schema, serializer, jitStack) {
...
export function getGeneratedJitFunctionFromClass(schema, serializer) {
    return schema.jit[serializer.fromClassSymbol];
}
export function getJitFunctionXToClass(schema, serializer) {
...
export function createXToClassFunction(schema, serializer, jitStack = new JitStack()) {
    const context = new Map();
    const prepared = jitStack.prepare(schema);
    const setProperties = [];
...
export function getXToClassFunction(schema, serializer, jitStack) {
    const jit = schema.jit;
    let fn = jit[serializer.toClassSymbol];
    if (fn)
        return fn;
...
export function getPartialXToClassFunction(schema, serializer) {
    const jit = schema.jit;
...
export function getPartialClassToXFunction(schema, serializer) {
    const jit = schema.jit;
...
export function createPartialXToClassFunction(schema, serializer) {
    const context = new Map();
    const jitStack = new JitStack();
    context.set('isGroupAllowed', isGroupAllowed);
    const props = [];
...
export function createPartialClassToXFunction(schema, serializer) {
    const context = new Map();
    const jitStack = new JitStack();
    context.set('isGroupAllowed', isGroupAllowed);
    const props = [];
