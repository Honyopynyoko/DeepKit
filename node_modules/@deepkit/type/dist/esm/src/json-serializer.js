 ...
 */
import { getEnumLabels, getEnumValues, getValidEnumValue, isObject, isValidEnumValue } from '@deepkit/core';
...
import { getClassSchema, getClassTypeFromInstance } from './model';
...
export function compilerToString(property, state) {
 ...
 */
export function classToPlain(classTypeOrSchema, target, options) {
 ...
 * ```
 */
export function plainToClass(classTypeOrSchema, data, options) {
 ...
 * ```
 */
export function validatedPlainToClass(classType, data, options) {
...
export function compilerToNumber(property, state) {
...
jsonSerializer.toClass.register('literal', (property, state) => {
    const literalValue = state.setVariable('_literal_value_' + property.name, property.literalValue);
    state.addSetter(literalValue);
});
jsonSerializer.toClass.register('uuid', (property, state) => {
    state.addCodeForSetter(`if ('string' === typeof ${state.accessor}) ${state.setter} = ${state.accessor};`);
});
jsonSerializer.fromClass.register('uuid', (property, state) => {
    state.addCodeForSetter(`if ('string' === typeof ${state.accessor}) ${state.setter} = ${state.accessor};`);
});
jsonSerializer.toClass.prepend('undefined', (property, state) => {
...
jsonSerializer.fromClass.prepend('undefined', (property, state) => {
    if (property.type === 'literal' && !property.isOptional) {
        const literalValue = state.setVariable('_literal_value_' + property.name, property.literalValue);
        state.addSetter(literalValue);
    }
...
jsonSerializer.toClass.prepend('null', (property, state) => {
    if (property.type === 'literal' && !property.isNullable) {
        const literalValue = state.setVariable('_literal_value_' + property.name, property.literalValue);
        state.addSetter(literalValue);
    }
});
jsonSerializer.fromClass.prepend('null', (property, state) => {
    if (property.type === 'literal' && !property.isNullable) {
        const literalValue = state.setVariable('_literal_value_' + property.name, property.literalValue);
        state.addSetter(literalValue);
    }
});
jsonSerializer.toClass.register('date', (property, state) => {
    state.addSetter(`new Date(${state.accessor});`);
});
jsonSerializer.toClass.register('boolean', (property, state) => {
...
jsonSerializer.toClass.register('enum', (property, state) => {
...
jsonSerializer.toClass.registerForBinary((property, state) => {
...
jsonSerializer.toClass.register('arrayBuffer', (property, state) => {
...
jsonSerializer.fromClass.register('bigint', (property, state) => {
...
export function convertArray(property, state) {
...
jsonSerializer.fromClass.register('class', (property, state) => {
...
jsonSerializer.toClass.register('class', (property, state) => {
...
jsonSerializer.toClass.register('union', (property, state) => {
...
jsonSerializer.fromClass.register('union', (property, state) => {
    let discriminator = [`if (false) { }`];
    const discriminants = [];
    for (const unionType of getSortedUnionTypes(property, jsonTypeGuards)) {
        discriminants.push(unionType.property.type);
