 ...
 */
import { arrayRemoveItem, capitalize, eachKey, eachPair, extractMethodBody, extractParameters, getClassName, isClass, isConstructable, isFunction, isObject, isPlainObject, toFastProperties } from '@deepkit/core';
import { t } from './decorators';
import { findCommonDiscriminant, findCommonLiteral } from './inheritance';
import { typedArrayMap, typedArrayNamesMap } from './types';
import { isArray } from './utils';
export function getSingleTableInheritanceTypeValue(classSchema) {
...
export function resolveClassTypeOrForward(type) {
    return isFunction(type) ? type() : type;
}
export function isPropertyValidator(object) {
...
export function forwardRef(forwardRef) {
 ...
 */
export function isRegisteredEntity(classType) {
    ...
    toString(optionalAffix = true) {
        ...
        if (this.type === 'class') {
            ...
            if (this.classTypeForwardRef) {
                ...
                if (resolved)
                    return getClassName(resolved) + affix;
...
export class ClassSchema {
    ...
    getSingleTableInheritanceDiscriminant() {
        ...
        if (!discriminant)
            discriminant = findCommonLiteral(this.subClasses);
        if (!discriminant) {
    ...
    extractForeignKeyToPrimaryKey(property, item) {
        ...
        if (pks.length === 1) {
            ...
            for (const pk of pks) {
                primaryKey[pk.name] = item[property.name + capitalize(pk.name)];
            }
        }
        return primaryKey;
    }
...
export function sliceClass(classType) {
    var _a;
 ...
 * ```
 */
export function mixin(...classTypes) {
    const constructors = [];
 ...
 */
export function hasClassSchemaByName(name) {
 ...
 */
export function getClassSchemaByName(name) {
 ...
 */
export function getKnownClassSchemasNames() {
 ...
 * @hidden
 */
export function getOrCreateEntitySchema(target) {
    ...
    if (!proto[classSchemaSymbol]) {
        ...
        while (currentProto && currentProto !== Object.prototype) {
            ...
            if (currentProto[classSchemaSymbol]) {
                found = true;
                const parent = currentProto[classSchemaSymbol];
...
export function hasClassSchema(target) {
    const proto = target['prototype'] ? target['prototype'] : target;
 ...
 */
export function getClassSchema(classTypeIn) {
 ...
 * ```
 */
export function createClassSchema(clazz, name = '') {
