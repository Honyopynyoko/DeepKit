...
exports.Workflow = exports.WorkflowError = exports.WorkflowStateSubject = exports.createWorkflow = exports.WorkflowDefinition = exports.WorkflowEvent = void 0;
...
class WorkflowDefinition {
    constructor(name, places, transitions = {}) {
        ...
        for (const placeName in this.places) {
            if (!this.places.hasOwnProperty(placeName))
                continue;
            const token = new event_1.EventToken(name + '.' + placeName, this.places[placeName]);
            this.tokens[placeName] = token;
            this['on' + core_1.capitalize(placeName)] = token;
        }
        ...
        core_1.toFastProperties(this.tokens);
        core_1.toFastProperties(this.next);
    }
    getEventToken(name) {
        if (!this.tokens[name])
    ...
    buildApplier(eventDispatcher) {
        ...
        compiler.context.set('getClassName', core_1.getClassName);
        const lines = [];
        ...
        for (const [place, eventType] of Object.entries(this.places)) {
            ...
            for (const listener of listeners) {
                if (event_1.isEventListenerContainerEntryCallback(listener)) {
                ...
                else if (event_1.isEventListenerContainerEntryService(listener)) {
                    ...
                    listenerCode.push(`
                    //${core_1.getClassName(listener.classType)}.${listener.methodName}
                    if (!event.isStopped()) {
            ...
            case ${stateString}: {
                ...
                if (!(event instanceof ${eventTypeVar})) {
                    throw new Error(\`State ${place} got the wrong event. Expected ${core_1.getClassName(eventType)}, got \${getClassName(event)}\`);
                }
...
exports.WorkflowStateSubject = WorkflowStateSubject;
class WorkflowError extends core_1.CustomError {
}
